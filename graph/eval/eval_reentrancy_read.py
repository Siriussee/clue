...
def reentrancy_read_amount_dependency_traverse(g: GraphTraversal, n_pairs_limit=50000):
    return (
        g.V().hasLabel('contractCall').as_('attacker')
            .repeat(T__.out('call')).emit().as_('victim')
            .inE('call').has('callTrace:type', P.neq('DELEGATECALL'))
            ...
            .inE('call').has('callTrace:type', P.neq('DELEGATECALL'))
            .where('victim', P.eq('re_victim')).by('address')
            .select('re_victim').in_('dcfg_call').as_('re_victim_dcfg')
            .select('victim')
            .limit(n_pairs_limit)
        # g.V().hasLabel('contractCall').as_('victim')
        #     .repeat(T__.out('call')).emit().as_('attacker')
        ...
        #     .select('re_victim')
            .local(
                ...
                .select('victim_flow').in_('dcfg_to_asset_flow').as_('victim_flow_dcfg')
                .select('attacker', 're_attacker', 'victim', 're_victim', 're_victim_dcfg', 'state_change', 'state_change_dcfg', 'victim_flow', 'victim_flow_dcfg').by(T__.element_map()).dedup()
                # .select('attacker', 'victim', 're_victim', 'state_change', 'state_change_dcfg', 'victim_flow', 'victim_flow_dcfg').by(T__.element_map()).dedup()
                # .select('victim_flow_dcfg', 'state_change_dcfg').by('dcfgId').dedup()
            )
    )
...
def reentrancy_read_control_dependency_traverse(g: GraphTraversal, n_pairs_limit=50000):
    return (
        g.V().hasLabel('contractCall').as_('attacker')
            .repeat(T__.out('call')).emit().as_('victim')
            .inE('call').has('callTrace:type', P.neq('DELEGATECALL'))
        ...
        #     .select('re_victim')
            .local(
                ...
                .select('attacker', 're_attacker', 'victim', 're_victim', 're_victim_dcfg', 'state_change', 'state_change_dcfg', 'victim_flow', 'victim_flow_dcfg').by(T__.elementMap()).dedup()
                # .select('attacker', 'victim', 're_victim', 'state_change', 'state_change_dcfg', 'victim_flow', 'victim_flow_dcfg').by(T__.elementMap()).dedup()
                # .select('victim_flow_dcfg', 'state_change_dcfg').by('dcfgId').dedup()
            )
    )
...
class ReentrancyEvaluation(Evaluation):
    ...
    def detect_attack(self, curr_epg_runner, attack_type, logfile=None) -> Tuple[float, bool]:
        if attack_type != 'reentrancy_read':
        ...
        # logger.info(g)
        try:
            for x in chain(reentrancy_read_amount_dependency_traverse(g.with_('evaluationTimeout', 300000)),
                        reentrancy_read_control_dependency_traverse(g.with_('evaluationTimeout', 300000))):
                re_victim_dcfg = DcfgId.from_str(x['re_victim_dcfg']['dcfgId'])
                ...
                if victim_flow_dcfg_id < state_change_dcfg_id:
                    # print('re_victim_dcfg: %s, victim_flow_dcfg_id: %s, <? %s' % (re_victim_dcfg, victim_flow_dcfg_id, re_victim_dcfg < victim_flow_dcfg_id))
                    if re_victim_dcfg < victim_flow_dcfg_id:
                        detected = True
...
if __name__ == '__main__':
    ...
    attack_type = 'reentrancy_read'

    ATTACK_TX_LIST = dataset.REENTRANCY_READ_TX_LIST
    if args.dataset == 'gas':
